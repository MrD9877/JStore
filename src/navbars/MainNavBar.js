...
import CartButton from "@/components/ui/cartbtn/CartButton";
        ...
        <nav className="hidden sm:flex bg-black text-white py-2 justify-between px-6">
          ...
          <div className="flex items-center gap-3">
            <span className="text-sm">
              {isLoggedIn ? (
                <button className="text-red-600 flex items-center gap-2 cursor-pointer" onClick={logout}>
                  Logout
                  <LogOut width={12} />
                </button>
              ) : (
            ...
            <svg
              onClick={() => setHeartColor((pre) => (pre === "white" ? "red" : "white"))}
              width="24"
              height="23"
              viewBox="0 0 24 23"
              fill="none"
