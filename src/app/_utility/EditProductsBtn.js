...
export default function EditProductsBtn({ product }) {
  ...
  const fetchChange = async () => {
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_SERVER_URL}/product`, { method: "PATCH", credentials: "include", body: JSON.stringify(data) });
  ...
  return (
    <>
      <div className="flex items-center">
        ...
        {/* increse btn  */}
        <button type="button" onClick={() => setCount((pre) => pre + 1)} id="increment-button" data-input-counter-increment="counter-input" className="inline-flex h-5 w-5 shrink-0 items-center justify-center rounded-md border focus:outline-none focus:ring-2  border-gray-600 bg-gray-700 hover:bg-gray-600 focus:ring-gray-700">
