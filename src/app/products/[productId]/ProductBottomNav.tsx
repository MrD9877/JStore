...
export default function ProductBottomNav({ product, inCart, selectedColor, selectedSize, outOfStock }: ProductBottomNavProps) {
  ...
  const changebutton = (state: boolean) => {
    if (state) {
  ...
  const handleAddcart = () => {
    changebutton(true);
    const color = selectedColor ? selectedColor : product.variants[0].color;
    const size = selectedSize ? selectedSize : product.variants[0].size;
    const variant = product.variants.filter((item) => item.color === color && item.size === size);
    if (variant.length === 0) return;
    if (variant[0].stock === 0) {
      return;
    }
    dispatch(addToCart({ product, sku: variant[0].sku }));
  };
  ...
  return (
    <>
      ...
      <Link href="/cart" style={cartBtnStyle.gotocart} id="gotocart" type="button" className="group py-4 px-5 rounded-full bg-indigo-50 text-indigo-600 font-semibold text-lg w-full flex items-center justify-center gap-2 shadow-sm shadow-transparent transition-all duration-500 hover:bg-indigo-100 hover:shadow-indigo-200">
        <Image className="mr-2" height={20} src="/images/check.gif" alt="gif" width={20} />
        Go to Cart
      </Link>
    </>
  );
