...
export default function UserProfileCard({ user }) {
  ...
  const handleLogout = async () => {
    try {
      const res = await fetch(`${process.env.SERVER_URL}/logout`, { credentials: "include" });
      ...
      <div className="h-full bg-gray-200 rounded-lg  sm:p-8">
        <div className="bg-white rounded-lg shadow-xl pb-8">
          ...
          <div className="w-full h-[250px]">
            <img onDoubleClick={handleAdminSignIn} src="https://vojislavd.com/ta-template-demo/assets/img/profile-background.jpg" className="w-full h-full object-cover rounded-tl-lg rounded-tr-lg" />
          </div>
