  ...
  const [error, submitAction, isPending] = useActionState(async (previousState, formData) => {
    ...
    try {
      ...
      if (addNewUser.status === 200) {
        toast("You are now signed in", true);
        router.push("/login");
      } else if (addNewUser.status === 400) {
        const res = await addNewUser.json();
        toast(res.msg, false);
      } else {
        toast(`error ${addNewUser.status}`, false);
      }
    } catch (err) {
      const error = err;
        ...
        <form style={{ background: "rgba(0, 10,20,0.9)" }} className="border rounded-xl max-w-sm mx-auto bg-white p-12 my-auto" action={submitAction}>
          ...
          <div className="mb-5">
            <label
              ...
              className="after:content-['*'] after:ml-0.5 after:text-red-500 block mb-2 text-sm font-medium text-white dark:text-white"
            >
              Repeat password
            </label>
